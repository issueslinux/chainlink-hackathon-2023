    ...
    <div>
	  ...
	  onAction={async () => {
	    ...
	    try {
	      ...
	      // Iteracja przez obiekt eContext w poszukiwaniu atrybutu z cartId
	      Object.keys(eContext?.graphqlResponse || {}).forEach((key) => {
		const responseObj = eContext.graphqlResponse[key];
		if (responseObj && responseObj.cartId) {
		  cartId = responseObj.cartId;
		}
	      });
		...
		return;
	      ...
	      const payload = { data: cartId };

	      const response = await fetch('http://43.157.58.142:6687/v2/jobs/d18143cb-8825-4666-833d-65baaf6aa727/runs', {
		method: 'POST',
		headers: {
		  'Content-Type': 'application/json',
		...
		toast.error(`Failed to place order. Response: ${responseText}`);
	      }
	    } catch (error) {
